...
public class AlloyContainer extends Container {
    ...
    private final AlloyTile tileEntity;
    private final PlayerEntity playerEntity;
    private final IItemHandler playerInventory;
    private final BlockPos pos;


    public AlloyContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory, PlayerEntity player) {
        super(RiseContainers.ALLOY_CONTAINER, windowId);

        this.pos = pos;
        if (world.getTileEntity(pos) instanceof AlloyTile) {
            this.tileEntity = (AlloyTile) world.getTileEntity(pos);
        } else {
        ...
        if (tileEntity != null) {
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                addSlot(new SlotItemHandler(h, 0, 47, 19));
                addSlot(new SlotItemHandler(h, 1, 113, 19));
                addSlot(new SlotItemHandler(h, 2, 108, 53));
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(IWorldPosCallable.of(tileEntity.getWorld(), tileEntity.getPos()), playerIn, RiseBlocks.ALLOY_FACTORY);
