...
public class PressContainer extends Container {
    ...
    private final PressTile tileEntity;
    private final PlayerEntity playerEntity;
    private final IItemHandler playerInventory;
    private final BlockPos pos;


    public PressContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory, PlayerEntity player) {
        super(RiseContainers.PRESS_CONTAINER, windowId);

        this.pos = pos;

        if (world.getTileEntity(pos) instanceof PressTile) {
            this.tileEntity = (PressTile) world.getTileEntity(pos);
        } else {
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(IWorldPosCallable.of(tileEntity.getWorld(), tileEntity.getPos()), playerIn, RiseBlocks.PRESS_FACTORY);
    }
...
//        playerEntity.sendMessage(new StringTextComponent("progress: " + this.tileEntity.getProgress()), playerEntity.getUniqueID());
...
//        int i = this.data.get(1); // current Time
//        int j = this.data.get(2); // TOTAL Time
//        return j != 0 && i != 0 ? i * 23 / j : 0;
//    }
}